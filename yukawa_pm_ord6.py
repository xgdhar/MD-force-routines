...
### new charge assignment
#@nb.autojit
#def charge_assgn_6_numba_wpart(rho_r,pos,Z,N,Mx,My,Mz,hx,hy,hz):
#    
#    n = 6
#    wx = np.zeros(n)
#    wy = np.zeros(n)
#    wz = np.zeros(n)
#    
#    wx_part = np.zeros((N,n))
#    wy_part = np.zeros((N,n))
#    wz_part = np.zeros((N,n))
#
#    rho_r.fill(0.0)
#
#    for ipart in range(N):
...
#        wz[5] = (1 + 10*z + 40*z**2 + 80*z**3 + 80*z**4 + 32*z**5)/3840
#        
#        # storing B-spline particle info
#        wx_part[ipart,:] = wx
#        wy_part[ipart,:] = wy
#        wz_part[ipart,:] = wz
#
...
#            izn += 1
#
#    return rho_r, wx_part, wy_part, wz_part
...
#@nb.autojit
#def field_assgn_6_numba_wpart(acc,wx_part,wy_part,wz_part,E_x_p,E_y_p,E_z_p,E_x_r,E_y_r,E_z_r,pos,Z,N,Mx,My,Mz,hx,hy,hz):
#    
#    E_x_p.fill(0.)
#    E_y_p.fill(0.)
#    E_z_p.fill(0.)
...
#def particle_mesh_fft_r(kappa,G_ew,pos,Z,N,G_k,kx_v,ky_v,kz_v,Mx,My,Mz,hx,hy,hz,Lx,Ly,Lz,p,mx_max,my_max,mz_max,rho_r,acc_f,E_x_p,E_y_p,E_z_p):
#    
#    V = Lx*Ly*Lz
#    M_V = Mx*My*Mz
#    
...
#    
#    E_x = M_V*np.fft.ifftn(E_kx_unsh)/V
#    E_y = M_V*np.fft.ifftn(E_ky_unsh)/V
#    E_z = M_V*np.fft.ifftn(E_kz_unsh)/V
#    
#    E_x_r = np.real(E_x)
#    E_y_r = np.real(E_y)
#    E_z_r = np.real(E_z)
